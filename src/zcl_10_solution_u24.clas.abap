CLASS zcl_10_solution_u24 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_10_SOLUTION_U24 IMPLEMENTATION.
...
*        DATA(carrier) = NEW lcl_carrier(  i_carrier_id = c_carrier_id ).
        DATA(carrier) = lcl_carrier=>get_instance(  i_carrier_id = c_carrier_id ).   "ubung 23
    ...
    catch cx_abap_auth_check_exception into data(exc_auth).              "ubung 18
...
*                    data = pass_flight->lif_output~get_output(  ) ).
                    data = pass_flight->get_output(  ) ).               "ubung 22
      ELSE.
...
*                    data = cargo_flight->lif_output~get_output(  ) ).           "ubung 22
                    data = cargo_flight->get_output(  ) ).           "ubung 22
      ELSE.
...
*    out->write( name = `Rental Overview`
*                data = lo_rental->lif_output~get_output(  ) ).
