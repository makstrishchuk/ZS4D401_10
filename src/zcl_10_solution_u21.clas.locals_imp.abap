...
*"* declarations
      INTERFACE lif_output.
        ...
        METHODS get_output RETURNING VALUE(r_result) TYPE tt_output.             "ubung 21
...
*    txt =  'Flug   &carrid& &connid&   am   &date& von &from& nach &to&'(005).
...
ENDCLASS.
      ...
      CLASS lcl_passenger_flight DEFINITION INHERITING FROM lcl_flight.
        ...
        PRIVATE SECTION.
          ...
          CLASS-DATA flights_buffer TYPE HASHED TABLE OF st_flights_buffer            "ubung 16
                  WITH UNIQUE KEY carrier_id connection_id flight_date
                  WITH NON-UNIQUE SORTED KEY sk_carrier COMPONENTS carrier_id.          "ubung 17
...
*      "Lokale Zeit umgerechnet auf UTC Zeit ---- Frankfurt
*      CONVERT DATE today TIME connection-departure_time
*      TIME ZONE connection-timzone_from                 "ubung 10
*      INTO UTCLONG DATA(departure_utclong).
*
*      "Lokale Zeit umgerechnet auf UTC Zeit ----New York
*      CONVERT DATE today TIME connection-arrival_time
*      TIME ZONE connection-timzone_to                   "ubung 10
...
*    ENDLOOP.
        ...
        METHOD get_flights_by_carrier.
          ...
          IF NOT line_exists( flights_buffer[ KEY sk_carrier carrier_id = i_carrier_id ] ). "ubung 14,17
...
*      SORT flights_buffer BY carrier_id connection_id flight_date. "ubung 14
...
*    ENDLOOP.
          ...
          LOOP AT flights_buffer ASSIGNING FIELD-SYMBOL(<fs>). "inline field symbols
          ...
          r_result = VALUE #( FOR <line> IN flights_buffer
                              USING KEY sk_carrier WHERE ( carrier_id = i_carrier_id )
                                      (
        ...
        ALIASES: t_output FOR lif_output~t_output,
          ...
          METHODS find_passenger_flight
            IMPORTING
              ...
              i_seats           TYPE i
            EXPORTING
              e_flight          TYPE REF TO lcl_flight      "ubung 20 up cast -> passenger_flight
              e_days_later      TYPE i.
          ...
          METHODS find_cargo_flight
            ...
            EXPORTING
              e_flight          TYPE REF TO lcl_flight            "ubung 20   up cast -> cargo_flight
              e_days_later      TYPE i.
...
*    DATA cargo_flights TYPE lcl_cargo_flight=>tt_flights.
          ...
          DATA flights TYPE lcl_flight=>tab.         "ubung 20 collection fur alle fluge
          DATA pf_count TYPE i.                      "ubung 20 anzahl der passagierfluge
          DATA cf_count TYPE i.                      "ubung 20 anzahl der frachtdluge
...
*    FIELDS name, currency_code         "delete ubung 11
...
*    name = carrier_id && ` ` && name.   "delete ubung 11

          DATA(passenger_flights) =                            "ubung 20
               lcl_passenger_flight=>get_flights_by_carrier(
                     i_carrier_id    = i_carrier_id ).
          pf_count = lines( passenger_flights ).
          DATA(cargo_flights) =                               "ubung 20
              lcl_cargo_flight=>get_flights_by_carrier(
                    i_carrier_id    = i_carrier_id ).
          pf_count = lines( cargo_flights ).

          LOOP AT passenger_flights INTO DATA(passflight).    "ubung 20 fluge in die collection
            APPEND passflight TO flights.                        "up cast
          ENDLOOP.


          LOOP AT cargo_flights INTO DATA(cargoflight).    "ubung 20 fluge in die collection
            APPEND cargoflight TO flights.                        "up cast
          ENDLOOP.
        ...
        METHOD find_passenger_flight.
          ...
          LOOP AT me->flights INTO DATA(flight)
            ...
            IF connection_details-airport_from_id = i_airport_from_id
             AND connection_details-airport_to_id = i_airport_to_id
             AND CAST lcl_passenger_flight( flight )->get_free_seats( ) >= i_seats.           "ubung 20
              DATA(days_later) = flight->flight_date - i_from_date.
        ...
        METHOD get_average_free_seats.
          ...
          r_result = REDUCE #( INIT i = 0
                               FOR <line> IN flights WHERE ( table_line IS INSTANCE OF lcl_passenger_flight ) "filter wegen colection die alle fluge enthalt
                               NEXT i = i + CAST lcl_passenger_flight( <line> )->get_free_seats( )    "ubung 20 downcast
