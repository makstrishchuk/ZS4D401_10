CLASS zcl_10_solution_u23 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_10_SOLUTION_U23 IMPLEMENTATION.
...
*        DATA(carrier) = NEW lcl_carrier(  i_carrier_id = c_carrier_id ).
    ...
    catch cx_abap_auth_check_exception into data(lx_excp).              "ubung 18
*    out->write( lx_excp->get_text(  ) ).
    out->write( 'Keine Berechtigung fur die Fluggesellschaft:' &&  c_carrier_id ).
    ENDTRY.
